...
export class TopPostsCardModalComponent implements OnInit {
 ...
 public onAgree(idChoice: string) {
  ...
  public onStartSurvey(){
    this.modalController.dismiss({action: "survey"});
  }
  ...
  public async onCardAnswered() {
    ...
    this.onCheckForAnswer(this.card.linkedCardId, this.userProfileId).subscribe(res => {
      ...
      loading.dismiss();
      if(this.alreadyAnswered == true && this.card.type != "Survey"){
        this.onInitialiseChart();
      }
      if (this.alreadyAnswered == false && this.endIdeaCard < this.date && this.card.answers > 0 && this.card.type != "Survey"){
        this.onInitialiseChart();
      }
  ...
  /**
   * method: onCheckAlreadyVoted
   * Check if the current user already answered that card
   */
  public async onCheckAlreadyVoted() {
    let loading = await this._loadingCtrl.create({
      spinner: 'crescent',
      showBackdrop: false,
      cssClass: 'transparent',
