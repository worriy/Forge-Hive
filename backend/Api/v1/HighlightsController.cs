...
namespace Hive.Backend.Api.HighlightsController
{
	...
	/// </summary>
	[Route("api/v1/HiveHighlightsController")]
    ...
    public class HighlightsController : Controller
	{
		...
		public async Task<IActionResult> GetBestContributor()
		{
			try
			{
                UserProfile user = await _UserProfileService.GetBestContributor();

                if (user == null)
                    return NoContent();

                var bestContributor = ConvertToBestContributor(user);
                return this.Ok(bestContributor);
            }
			catch (Exception xcp)
			{
                //log exception
