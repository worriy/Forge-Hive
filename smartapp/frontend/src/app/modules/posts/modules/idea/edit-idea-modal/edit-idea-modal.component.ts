...
export class EditIdeaModalComponent implements OnInit, OnDestroy {
    ...
    this.getTargetableGroups().then(() => {
      this._postsController.getEditablePost(this.post.id).subscribe(data => 
      {
        ...
        this.ideaForm = this.formBuilder.group({
          content: [this.post.content, Validators.compose([Validators.required, Validators.maxLength(90)])],
  ...
  /**
   * Check the edited Idea's consistency
   */
  public async checkInformation(){
    const publicationDate = new Date(this.ideaForm.controls.publicationDate.value);
    const endDate = new Date(this.ideaForm.controls.endDate.value);
  ...
  public async saveChanges() {
    ...
    let editVM: EditPost = {
      ...
      publicationDate: this.ideaForm.value.publicationDate,
      endDate: this.ideaForm.value.endDate,
      targetGroupsIds: this.ideaForm.value.targetGroups,
      pictureId: null
    };
  ...
  public checkSelectedGroups(){
    const selectedGroups = this.ideaForm.controls.targetGroups.value;
    if(selectedGroups.length == 0 || (selectedGroups.length > 1 && selectedGroups.find(groupId => groupId === this.publicGroupId)))
    {
      this.ideaForm.controls.targetGroups.setValue([this.publicGroupId], { emitEvent: false });
      return;
    }
  }
