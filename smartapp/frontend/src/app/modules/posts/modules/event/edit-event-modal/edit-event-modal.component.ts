...
export class EditEventModalComponent implements OnInit {
  ...
  /**
   * Check the edited Event's consistency
   */
  public async checkInformation(){
    const publicationDate = new Date(this.eventForm.controls.publicationDate.value);
    const endDate = new Date(this.eventForm.controls.endDate.value);
  ...
  public checkSelectedGroups(){
    const selectedGroups = this.eventForm.controls.targetGroups.value;
    if(selectedGroups.length == 0 || (selectedGroups.length > 1 && selectedGroups.find(groupId => groupId === this.publicGroupId)))
    {
      this.eventForm.controls.targetGroups.setValue([this.publicGroupId], { emitEvent: false });
      return;
    }
  }
